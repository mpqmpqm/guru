...
export const speakArgsSchema = z.object(speakArgShape).strict();
...
export async function runSpeakTool(
  sessionId: string,
  args: SpeakArgs
): Promise<string> {
  ...
  const result = `spoke ${(speakingMs / 1000).toFixed(1)}s | ${ratio} | ${getTimeInfo(sessionId)}`;
